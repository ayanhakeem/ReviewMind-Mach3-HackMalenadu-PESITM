    ...
    Papa.parse(file, {
      ...
      complete: (results) => {
        ...
        if (data && data.length > 0) {
          ...
          const imageColumn = columns.find(c => /image|photo|pic|media|img|url/i.test(c) && c !== bestColumn);
          
          const normalizedData = data.map(row => {
              let images = [];
              if (imageColumn && row[imageColumn]) {
                  // Handle comma-separated lists of images or single strings
                  const rawImg = row[imageColumn].toString();
                  images = rawImg.includes(',') ? rawImg.split(',').map(s => s.trim()) : [rawImg.trim()];
                  // Filter for only valid-looking URLs
                  images = images.filter(url => url.startsWith('http') || url.startsWith('data:image'));
              }
